...
docker buildx build --no-cache --progress=plain --push --platform linux/amd64,linux/arm64/v8 --rm -t wujidadi/ubuntu-tuned:3.3 -t wujidadi/ubuntu-tuned:latest . 2>&1 | tee $D/docker-build-ut-3.3.log

# Create testing container
docker run -d -p 50000:80 -it --name Test wujidadi/ubuntu-tuned:3.3
