...
docker buildx build --no-cache --progress=plain --push --platform linux/amd64,linux/arm64/v8 --rm -t wujidadi/ubuntu-tuned:2.13 -t wujidadi/ubuntu-tuned:latest . 2>&1 | tee $D/docker-build-ut-2.13.log

# Create test container and delete it finally
docker run -d -p 50000:80 -it --name Test wujidadi/ubuntu-tuned:2.13
